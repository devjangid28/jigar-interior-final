  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (heroLogo) {
          // Scale: 1.5 → 0.12 (header size)
          const svgScale = 1.1 - t * (0.9 - 0.12);
          // Move up from center (50vh - 9vw) to header position (~40px from top)
          const translateY = -t * 100;
          // Opacity: fade out in last 30%
          const opacity = t > 0.7 ? 1 - ((t - 0.7) / 0.3) : 1;

          heroLogo.style.transform = `scale3d(${svgScale}, ${svgScale}, 1) translate(0, ${translateY}%)`;
    ...
    <>
      <header id="Header" className={menuOpen ? '--with-bg' : ''}>
        ...
        <nav className="__menu" role="menu">
          <a className="link-circle" href="#projects" role="menuitem">
            <span>Projects</span>
          </a>
          <a className="link-circle" href="#services" role="menuitem">
            <span>Services</span>
          </a>
